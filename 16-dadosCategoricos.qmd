...
minha_tab <- flextable(df) %>%
  set_header_labels(
    ...
    Total = "Total") %>%
  autofit() %>%
  add_footer_lines(value = "Nota: Baixo Peso = peso ao nascer < 2500g") %>%
...
A partir desses resultados, pode-se inferir que a menor taxa de infecção ocorre no grupo do primeiro quartil e é significativamente diferente em relação a taxa de infecção do maior quartil, mas sem indicação para os grupos intermediários. 
...
As respostas podem ser resumidas na @tbl-tabmcnemar: 
