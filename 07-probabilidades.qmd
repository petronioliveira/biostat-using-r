...
```{r #fig-moeda20, echo = FALSE, out.height = '80%', out.width = '80%', fig.align = 'center', fig.cap="20 lançamentos seguidos de uma moeda", fig.pos="H"}
...
```{r #fig-nmoeda, echo = FALSE, out.height = '80%', out.width = '80%', fig.align = 'center', fig.cap="Proporção à medida que N aumenta em lançamentos de moedas", fig.pos="H"}
...
A definição frequentista também pode ser aplicada no cotidiano. Utilizando a altura de 1368 mulheres, uma medida numérica contínua, incluída no conjunto de dados `dadosMater.xlsx` (veja @sec-mater). Essas alturas serão selecionadas e colocadas em um objeto, denominado `dados`.
...
Observe que o valor de 1,60 m está muito próximo da média e isto é um indicativo de que essa variável tem uma distribuição praticamente simétrica. Sabendo a distância, em números de desvios padrão, que 1,60 m está da média, qual a probabilidade de encontrar, na maternidade do HGCS [^07-probabilidades-1], uma parturiente que tenha exatamente esta altura?\
Para responder a essa pergunta, será usada a função `pnorm()` (veja adiante na @sec-dnp) que utiliza o escore *Z*, a média e o desvio padrão para encontrar essa proporção que, multiplicada por 100, fornece a percentagem.

[^07-probabilidades-1]: Hospital Geral de Caxias do Sul, Hospital de Ensino da Universidade de Caxias do Sul, RS
...
O *R* por padrão retorna números grandes como notação científica. O resultado dessa operação é um número tão grande que para escrevê-lo sem este tipo de notação, seriam necessários 127 dígitos decimais. O resultado não caberia em apenas uma linha. Ficaria assim, suprimindo a notação científica [^07-probabilidades-2]:

[^07-probabilidades-2]: Para remover a notação científica, usar a função options (scipen = 999) e, para desfazer essa ação, trocar o 999 por 0.
...
```{r #fig-curvanormal, echo = FALSE, warning=FALSE, out.width="70%", out.height="70%", fig.align="center", fig.cap="Curva normal.", fig.pos="H"}
...
```{r #fig-threecurves, echo = FALSE, warning=FALSE, out.width="70%", out.height="70%", fig.align="center", fig.cap="Curvas normais com modificação dos parâmetros.", fig.pos="H"}
curve (dnorm (x, 
              mean=0, 
              sd=1), 
       col="dodgerblue3", 
...
Se for usado $mean = 0$ e $sd = 1$, o valor de q = Z, caso contrário, toma-se os valores da média, o desvio padrão da população e o valor de *x*. Com esta função, é possível responder a pergunta feita anteriormente em relação a probabilidade de encontrar uma mulher com mais de 1,725m, equivalente a `r round(Z, 2)` desvios padrão acima da média, em uma população com média = `r resumo$media` e desvio padrão = `r resumo$dp`.
...
Para representar graficamente essa pequena probabilidade, será construída uma curva com essa pequena área sombreada, colorida em vermelho. Para isso, será feito uso de uma função própria, denominada `normal_area()`. Ela pode ser obtida [aqui](https://github.com/petronioliveira/Arquivos/blob/main/normal_area.R) para ser baixada em seu diretório de trabalho para uso posterior [^07-probabilidades-3].

[^07-probabilidades-3]: Veja na Seção @sec-funcpropria.

A @fig-prob1725 representa com clareza esta pequena probabilidade. Foi usada a função `text()` para escrever o valor da probabilidade.

```{r #fig-prob1725, warning=FALSE, out.width="70%", out.height="70%", fig.align="center", fig.cap="Probabilidade de encontrar mulheres com mais de 1,725m", fig.pos="H"}
...
```{r  #fig-pdf, warning=FALSE, out.width="70%", out.height="70%", fig.align="center", fig.cap="Função densidade de probabilidade.", fig.pos="H"}
plot(valores_d,
     type = "l",                          
     lwd = 2,                             
     col = "steelblue",                  
...
Emn sequência, serão construídos histogramas (@fig-pdf1), onde se pode observar que, aumentando o número de observações, tem-se gráficos que irão progressivamente se aproximando da verdadeira função de densidade normal.\
...
```{r #fig-pdf1, warning=FALSE, out.width="100%", out.height="800%", fig.align="center", fig.cap="Histogramas construídos com amostras geradas pela função rnorm.", fig.pos="H"}
...
```{r #fig-histdp, echo= FALSE, warning=FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Histograma com curva de densidade de probabilidade sobreposta", fig.pos="H"}
...
Como visto na @sec-aplicafreq, a variável `dados$altura` tem uma distribuição praticamente simétrica. Usando esses dados ($X\sim N(\mu=1,598,\sigma=0,065)$), pode-se calcular probabilidades, dadas pela área sob a curva.
...
```{r #fig-normal954, echo=FALSE, message = FALSE, warning=FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Probabilidade de alturas entre 1,47 e 1,73m.", fig.pos="H"}
source("dados/normal_area.R")
...
A probabilidade de alturas entre 1,47 m e 1,73m é igual a 95,4% (@fig-normal954).
...
```{r #fig-normal192, echo=FALSE, message = FALSE, warning=FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Probabilidade do tempo de cirurgia de LCA menor ou igual a 100 minutos.", fig.pos="H"}
source("dados/normal_area.R")
...
De acordo com a distribuição, 1,92%% das cirurgias irão demandar quantidade de tempo menor do que 100 minutos (@fig-normal192).
...
```{r #fig-normal134, echo=FALSE, message = FALSE, warning=FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Probabilidade do tempo de cirurgia de LCA maior ou igual a 160 minutos.", fig.pos="H"}
source("dados/normal_area.R")
normal_area(media = 129, dp = 14, 
            linf = 160, lsup = 200, 
            cor = "tomato", lwd = 2 )
...
De acordo com a distribuição, 1,34% das cirurgias irão demandar quantidade de tempo $\ge 160$ minutos (@fig-normal134). Ou seja, é uma probabilidade muito pequena!
...
```{r #fig-normal649, echo=FALSE, message = FALSE, warning=FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Probabilidade de uma mulher com 150 cmm pertencer a uma população de média igual a 160 cmm.", fig.pos="H"}
source("dados/normal_area.R")
...
Na população 1, apenas 6.5% das mulheres tem altura $\le 1,50$ m (@fig-normal649). Em outras palavras, existe pouca probabilidade dessa mulher pertence à população 1.
...
```{r #fig-normal935, echo=FALSE, message = FALSE, warning=FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Probabilidade de uma mulher com 150 cmm pertencer a uma população de média igual a 140 cmm.", fig.pos="H"}
source("dados/normal_area.R")
...
Na população 2, 93,5% das mulheres tem altura $\le 1,50$ m (@fig-normal935). Concluindo, ela pode pertencer a qualquer uma das populações. Pode ser uma mulher alta da população 2 ou uma “baixinha” da população 1!
...
A probabilidade de sucesso [^07-probabilidades-4], em uma distribuição binomial, é dada pela fórmula:

[^07-probabilidades-4]: *Sucesso*, aqui, não está no sentido de vitória, êxito, triunfo, glória e sim com a conotação de obter o desfecho esperado. Por exemplo, se uma moeda é lançada e se espera obter cara, sucesso significa um resultado igual a cara.
...
Isso corresponde a soma das probabilidades de nascer nenhum menino, um menino, dois meninos e três meninos (@fig-menino3). Isto é calculado pela equação $P(X = x)$, vista anteriormente.\
Colocando no *R*:
...
```{r #fig-menino3, echo=FALSE, warning=FALSE, out.width="70%", out.height="70%", fig.align="center", fig.cap="Distribuição binomial, mostrando a P (x < 4) com n = 5 e p = 0.50 ", fig.pos="H"}
# Gráfico
bp <- barplot(Fx,
              names.arg=0:5,
...
Para fazer uma simulação de 1000 amostras, aleatoriamente, de tamanho 5 e com probabilidade de nascer menino igual a 0,50, usa-se [^07-probabilidades-5]:

[^07-probabilidades-5]: Deve ser especificado uma "semente" (seed) antes de executar a função, senão será obtido um conjunto diferente de observações aleatórias a cada execução. Teste para verificar
...
```{r #fig-menino5, echo=FALSE, warning=FALSE, out.width="70%", out.height="70%", fig.align="center", fig.cap="Distribuição binomial para P (x = 4) com n = 5 e p = 0,50", fig.pos="H"}
bp <- barplot(Fx,
              names.arg=0:5,
