...
Idealmente, um teste de diagnóstico deveria ter altos níveis de sensibilidade e especificidade. No entanto, isso não é possível, pois existe um balanço entre sensibilidade e especificidade. À medida que a especificidade aumenta, a sensibilidade diminui e vice-versa. As curvas ROC podem ser usadas para identificar um ponto de corte em uma medição contínua que maximize a sensibilidade e a especificidade (veja a @sec-rocurve).
...
[Leitura e observação do conjunto de dados]{.underline}
...
[Construção de uma tabela de contingência 2\$\times\$2]{.underline}
...
[Cálculo da sensibilidade e da especificidade]{.underline}
...
```{r #fig-fagan, echo = FALSE, out.width = '60%', fig.align = 'center', fig.cap="Nomograma de Fagan", fig.pos="ht"}
...
```{r #fig-diabetes, echo = FALSE, out.width = '80%', fig.align = 'center', fig.cap="Populações de indivíduos normais (curva em azul) e diabéticos (curva em vermelho)", fig.pos="ht"}
curve(dnorm(x, 90,20), 
      from = 0, to = 300, 
      ylim = c(0, 0.02),
      col = "royalblue", 
...
[Leitura e observação dos dados]{.underline}
...
```{r #fig-status123, echo = FALSE, out.width = '60%', fig.align = 'center', fig.cap="Resultado do teste vs doença.", fig.pos="ht"}
...
```{r #fig-roc, message=FALSE, echo=TRUE, fig.align = 'center', out.width="80%", out.height="80%", fig.cap="Curvas ROC para os Testes 1, 2 e 3.", fig.pos="ht"}
...
[Interpretação do resultado]{.underline}
...
```{r #fig-roc3, message=FALSE, echo=TRUE, out.width="70%", fig.align = 'center',  fig.cap="Curvas ROC para os Testes 1, 2 e 3.", fig.pos="ht"}
...
```{r #fig-incidencia, echo = FALSE, out.width = '75%', fig.align = 'center', fig.cap="Incidência", fig.pos="ht"}
...
```{r #fig-incidencia1, echo = FALSE, out.width = '65%', fig.align = 'center', fig.cap="Pessoas-tempo (estudo epidemiológico hipotético).", fig.pos="ht"}
...
#| message: FALSE
#| warning: FALSE
#| tbl-cap: "Tabela de contingência 2 x 2"
...
#| message: FALSE
#| warning: FALSE
#| tbl-cap: "Força de associação do RR comparado com a OR."

magni <- c("insignificante", "pequena", "moderada", "grande", "muito grande", "quase perfeita", "perfeita")
or <- c("1,0", "1,5", "3,5", "9,0", "32", "360", "infinito")
rr <- c("1,0", "1,2", "1,9", "3,0", "5,7", "19", "infinito")

df <- data.frame(or, rr, magni)
...
#| message: FALSE
#| warning: FALSE
#| tbl-cap: "Taxa de ataque de gastrenterite com sorvete de baunilha - Oswego"
...
#| message: FALSE
#| warning: FALSE
#| tbl-cap: "Physicians' Health Study, componente aspirina e IAM"
...
[Matriz dos dados]{.underline}[^18-epidemiologia-6]
...
#| message: FALSE
#| warning: FALSE
#| tbl-cap: "Physicians' Health Study, componente aspirina e AVC." 
...
[Carregar o conjunto de dados]{.underline}
...
[Construir uma tabela tratamento vs evento]{.underline}
...
[Calcular as estimativas de sobrevida de Kaplan-Meier para a construção da Curva de Sobrevida de cada tratamento]{.underline}
...
[Visualização da curva de sobrevida]{.underline}

Pode-se visualizar a curva (@fig-survival) de uma maneira simples, utilizando a função `plot()` do pacote básico do R:

```{r #fig-survival, echo=TRUE, out.width="70%", fig.align = 'center',  fig.cap="Curva de sobrevida comparando dois grupos de tratamento.", fig.pos="ht"}
...
```{r #fig-survival1, echo=TRUE,warning = FALSE, message = FALSE,  out.width="70%",  fig.align = 'center',  fig.cap="Curva de sobrevida comparando dois grupos de tratamento, usando ggsurvplot().", fig.pos="ht"}
ggsurvplot (tabsurv,
            pval = TRUE, 
            conf.int = FALSE,
            risk.table = "abs_pct",
...
```{r #fig-curvalog, echo = FALSE, out.width = '50%', fig.align = 'center', fig.cap="Curva logística geral", fig.pos="H"}
...
3.  **Independência dos erros/resíduos**: os erros (também chamados de resíduos) não devem possuir correlação. Este pressuposto pode ser testado pelo teste de Durbin-Watson (veja a @sec-dw) e observando o gráfico quantil-quantil (Q-Q) dos resíduos padronizados.
...
```{r #fig-titanic, echo = FALSE, out.width = '50%', fig.align = 'center', fig.cap="Titanic parado em Queenstown, 11 de abril.de 1912", fig.pos="H"}
...
Ainda há necessidade de outras transformações, pois verifica-se que existem variáveis classificadas como numéricas (`classe`, `sexo` e a variável desfecho `sobreviveu`), mas que são fatores. Esta modificação será realizada, usando a função `mutate()` do pacote `dplyr`(veja a @sec-mater):
...
Para a 3ª classe, comparada à 1ª classe, há uma diminuição de `r (1 - odds_ratio1[3, 1]) * 100`% na chance de sobrevivência. Da mesma forma a 2ª classe, comparada à 1ª classe, tem uma diminuição de `r (1 - odds_ratio1[2, 1]) * 100`%% na chance de sobrevivência [^18-epidemiologia-11].

[^18-epidemiologia-11]: Veja mais detalhes sobre *Odds Ratio* na @sec-or.
...
```{r #fig-forest, message=FALSE, warning=FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Gráfico de Floresta dos Coeficientes de uma Regressão Logística em formato Odds Ratio", fig.pos="H"}
library(sjPlot)
...
```{r #fig-interacao, message=FALSE, warning=FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Gráfico de linha da interação sexo:classe", fig.pos="H"}
...
```{r #fig-taxas, message=FALSE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Proporção de sobrevivência por sexo e classe", fig.pos="H"}
...
[Predição da variável desfecho nos modelos]{.underline}
...
[Crição de um objeto ROC]{.underline}
...
[Plotar a curva ROC]{.underline}

```{r #fig-comparemod, message=FALSE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Comparação das curvas ROC(modelo sem interação = preto; modelo com interação vermelho)", fig.pos="H"}
plot(roc2, 
     main = "Curva ROC", 
     print.auc = TRUE, 
     legacy.axes=TRUE,
...
```{r #fig-compareroc, message=FALSE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Desempenho do modelo treino e do modelo teste)", fig.pos="H"}
...
```{r #fig-linearidade, echo = FALSE, out.width = '100%', fig.align = 'center', fig.cap="Avaliação do modelo com foco na linearidade", fig.pos="H"}
...
```{r #fig-linearidade2, echo = FALSE, out.width = '100%', fig.align = 'center', fig.cap="Avaliação do modelo com interação com foco na linearidade", fig.pos="H"}
...
```{r #fig-graph5, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Gráfico diagnóstico dos resíduos e pontos de alavancagem.", fig.pos="H"}
