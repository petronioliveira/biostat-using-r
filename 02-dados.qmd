...
```{r #tbl-var, echo=FALSE, message=FALSE, warning=FALSE}
library(dplyr)  
knitr::kable(tabela,              
             "html",             
...
Em decorrência do acaso, diferentes amostras de uma mesma população fornecem resultados diferentes. Este fato deve ser levado em consideração ao usar uma amostra para fazer inferência sobre uma população. Este fenômeno é denominado de **variação amostral** ou **erro amostral** (Consulte também o @sec-cap9) e é a essência da estatística. O grau de certeza na inferência estatística depende da representatividade da amostra.
...
```{r #fig-pop, echo = FALSE, out.width = '60%', fig.align = 'center', fig.cap="População, amostra e inferência estatística", fig.pos="H"}
...
A maneira mais fácil de dizer se os dados são numéricos é verificar se eles têm unidades ligadas a eles, tais como: g, mm, ºC, ml, número de úlceras de pressão, número de mortes e assim por diante. Se não, podem ser ordinais ou nominais -- ordinais se os valores podem ser colocados em ordem. A @fig-caminho é uma ajuda para o reconhecimento do tipo de variável @bowers2008scratch.

```{r #fig-caminho, echo = FALSE, out.width = '65%', fig.align = 'center', fig.cap="Caminho para identificar o tipo de variável", fig.pos="H"}
