...
Para reduzir o número de dígitos decimais, na saída do resultado, pode-se colocar a função `mean()`, dentro da função `round()`[^06-descrevendodados-1], atribuindo o resultado da função a um objeto, por exemplo `media`.

[^06-descrevendodados-1]: Usa-se esta sintaxe: `round(x, digits = 0)`, onde `x` é o numero que se quer arredondar e `digits` é número de casas decimais. O padrão é `0`, ou seja, arredonda para o inteiro mais próximo.
...
Se os dados forem nominais, a ordenação das categorias é arbitrária, costuma-se colocar em primeiro lugar a maior frequência (@tbl-droga) , colocando-os em categorias ordenadas @oliveira2022tab.
...
Esta uma tabela simples que serve para visualizar as informações. Não serve para publicações. Para obter uma tabela simples, mas muito mais profissional (@tbl-tisch) pode ser utilizada a função `kable()` do pacote `knitr` @xie2018knitr. Esta função possui um grande número de argumentos para personalizar a aparência das tabelas[^06-descrevendodados-2]:

[^06-descrevendodados-2]: Para maiores detalhes: https://haozhu233.github.io/kableExtra/awesome_table_in_pdf.pdf ou consulte a ajuda digitando no Console `?kable()` ou `help(kable)`. Um pacote adicional, `kableExtra` @zhu2021construct, permite opções de formatação simples, melhorando o aspecto da tabela, utilizando-se o operador pipe `%>%`. O pacote `kableExtra` foi projetado para estender a funcionalidade básica das tabelas produzidas usando `knitr::kable()`. Podem ser acrescentadas várias das suas funções como `kable_styling()` ou `kable_classic()` para especificar estilos à tabela, como extensão da tabela, alinhamento, tipo e tamanho da fonte

```{r #tbl-tisch}
#| tbl-cap: "Distribuição das puérperas por faixa etária, Hospital Geral, Caxias do Sul, RS, 2008"
#| echo: TRUE
...
Para a maioria dos dados, é recomendado e 8 a 20 classes, isto é, 8 $\le$ k $\le$ 20. Com poucas classes, perde-se precisão e, com muitas classes, a tabela torna-se muito extensa. Baseado na regra de Sturges , é sugerido usar a recomendação da @fig-sturges @arango2009classes.

```{r #fig-sturges, echo = FALSE, out.width = '70%', fig.align = 'center', fig.cap="Número de classes baseado em Sturges", fig.pos="H"}
...
Assim, é recomendado um ganho de peso total adequado de 12,5 kg a 18 kg para as gestantes classificadas como baixo peso; de 11,5 kg a 16,0 kg para as classificadas como peso adequado; de 7,0 a 11,5 kg nas classificadas com sobrepeso; e de 5,0 a 9,0 kg nas obesas [@rasmussen2009weight].
...
Colocando em um formato mais científico, tem-se uma tabela ( @tbl-aripaka)) bem mais elegante sobre o estado nutricional pré-gestacional:

```{r #tbl-aripaka}
#| tbl-cap: "Estado nutricional pré-gestacional das parturientesa, Hospital Geral, Caxias do Sul, RS, 2008"
#| echo: TRUE
...
Em uma consulta, entre estudantes de Medicina, foi perguntado a sua opinião em relação a este tipo de gráfico. A pergunta feita foi: “O que você sente ao ver um gráfico de pizza em um artigo científico?” As alternativas para a resposta eram quatro (ódio, irritação, indiferença, amor). O resultado do inquérito está na @tbl-pizza.

```{r #tbl-pizza}
#| tbl-cap: "Sentimento dos alunos de Medicina em relação ao gráfico de pizza, UCS, 2012."
#| echo: false

df2 <- data.frame(Sentimento = c("Odeiam", "Não gostam",
...
knitr::kable(df2,
             format = "html",
             col.names = c("Sentimento", "f", "fr", "fp (%)", "Fp (%)"),
             align = c('l', 'c', 'c', 'c', 'c')) %>%
 kableExtra::kable_styling(
    bootstrap_options = c("striped", "hover", "condensed"),
    full_width = FALSE, 
...
Os valores da coluna de frequência absoluta (*f*) da @tbl-pizza serão usados como o argumento *x*. Ele informa a área (proporção de cada fatia). Os rótulos das fatias são escritos com a função concatenar `c()`.

```{r #fig-pie, out.width="90%", out.height="90%", fig.align='center', fig.cap="Gráfico de Pizza: Opinião dos estudantes de Medicina.", fig.pos="H"}
pie(x = c(6, 12, 14, 8),
    labels = c("Odeiam", "Não gostam", "Indiferentes", "Amam"))
```

As cores que aparecem na @fig-pie foram escolhidas pelo *R*, usando o seu padrão. Entretanto, elas podem ser customizadas, especificando-as pelo nome colocado entre parênteses. Por exemplo, `col = "red"` e se for mais de uma cor usar a função concatenar, `col = c(“gray58“, “yellow4”, “cyan”, “tomato”)`. As cores também podem se denotadas pelo sistemas RGB ou hexadecimal. A sigla RGB representa as cores primárias em inglês (*Red*, *Green*, *Blue*). O código hexadecimal da cor branca é #FFFFFFF, da gray58 é #949494, da yellow4 é #999900, da cyan é #00FFFFe da tomato é #FF6347 (@fig-pie2).

```{r #fig-pie2, out.width="90%", out.height="90%", fig.align="center", fig.cap="Figura anterior com cores personalizadas.", fig.pos="H"}
...
As cores parecem "espetaculosas", mas o objetivo foi de criticar os gráficos tipo pizza. Se o leitor quiser insistir no seu uso e com um gráfico em 3D (@fig-pie3d), pode-se instalar o pacote `plotrix` @lemon2015package e carregar a função `pie3D()`. Os argumentos são praticamente os mesmos do gráfico simples. Acrescenta-se `radius = 0.9` que muda o raio da pizza e `explode = 0.1` que determina o afastamento das fatias (0, as mantém juntas). Além disso, como o gráfico exibe rótulos com textos muito grandes, usa-se o argumento `labelcex = 1`. Como qualquer função nova, basta clicar na tecla *Tab*, dentro da mesma, que aparece um menu com as alternativas de argumentos.

```{r #fig-pie3d, out.width="90%", out.height="90%", fig.align='center', fig.cap="Gráfico de Pizza: Opinião dos estudantes de Medicina.", fig.pos="H"}
library (plotrix)
...
Para a construção do gráfico de barras simples da @fig-barra1, será utilizada a variável `categIdade`, anteriormente criada (@sec-tabfreq), a partir do conjunto de dados `dadosMater.xlsx`.

```{r #fig-barra1, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico de barra simples.", fig.pos="H"}
barplot(table(mater$categIdade))
```

Observando a @fig-barra1, verifica-se que não existem rótulos nos eixos *x* e *y* e o eixo *y* tem um tamanho inferior a barra mais alta. Estes e outros problemas podem ser resolvidos modificando-se ou acrescentando outros argumentos na função `barplot()`. Existem vários argumentos e para conhece-los melhor pesquise no *Help* do RStudio. Em um gráfico de barra simples são suficientes as seguintes modificações que irão resultar na @fig-barra2:

-   Para corrigir a amplitude do eixo *y*, existe o argumento `ylim = c(lim inf, lim sup)`. Na @tbl-tisch, observa-se que a frequência máxima é de 992, assim estende-se até 1000, bem próximo da frequência da categoria, acrescentando `ylim = c (0,1000)`, separado por vírgulas de outros argumentos.
...
-   Para a cor das barras, use o argumento `col = (“cor”)`. Escolha a cor entre as 657 opções, ou deixe o padrão cinza (*grey*). O argumento `col.axis = “cor”` controla a cor dos valores dos eixos (veja também @sec-cores).
...
```{r #fig-barra2, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico de barra simples modificado.", fig.pos="H"}
barplot(table(mater$categIdade), 
        ylim = c (0,1000), 
        col= "tomato", 
        border = "black", 
...
Para que as barras fiquem horizontais como na @fig-barrah, usa-se o argumento `horiz=TRUE`:

```{r #fig-barrah, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico com barras horizontais.", fig.pos="H"}
barplot(table(mater$categIdade), 
        xlim = c (0,1000), 
        col= "tomato", 
        border = "black", 
...
Além das modificações realizadas, pode-se fazer outras para tornar o gráfico mais informativo . Por exemplo, colocar as frequência de cada barra no topo das mesmas (@fig-barra3):

-   **1º Passo**: Criar um gráfico de barras , colocando-o em um objeto qualquer, por exemplo, *x* [^06-descrevendodados-4], que conterá o eixo *X* do centro de cada uma das barras. Para verificar isso, basta executar o objeto *x*;
-   **2º Passo**: colocar a tabela `table(mater$idadeCateg)` como um objeto *y* [^06-descrevendodados-5], onde estarão as frequências absolutas);
-   **3º Passo**: usar a função `text()` para colocar os valores.Consulte o *Help* para maiores detalhes desta função.

[^06-descrevendodados-4]: Foi escolhido o nome *x*, porque lembra o eixo *X*, onde estão as barras

[^06-descrevendodados-5]: para lembrar o eixo *Y*

```{r #fig-barra3, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico de barra simples com frequências no topo.", fig.pos="H"}
...
```{r #fig-stacked, out.width="90%", out.height="90%", fig.align='center', fig.cap="Gráfico de barras empilhadas.", fig.pos="H"}
...
É igual a anterior, apenas com o argumento `beside = TRUE` (@fig-side).

```{r #fig-side, out.width="90%", out.height="90%", fig.align='center', fig.cap="Gráfico de barras lado a lado", fig.pos="H"}
barplot(tabFumo2,
        beside = TRUE,
        ylim = c(0, 1000),
        xlab="Faixa Etária", 
...
A variável `mater$para`, número de filhos anteriores ao atual, é uma variável numérica discreta e, para representá-la, o mais adequado é usar um gráfico de barras simples (@fig-discreta).

```{r #fig-discreta, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico de barras para uma variável discreta", fig.pos="H"}
...
O próximo passo é criar um objeto, denominado `barras`, que irá receber as médias dos pesos dos recém-nascidos masculinos e femininos, que representam a altura das barras. Este objeto servirá de base para a construção de um gráfico de barras que será recebido por outro objeto, `bp`. Finalmente, coloca-se os limites inferiores e superiores para cada sexo, usando os valores calculados pela função `summarise()` que junto com o objeto bp constituem-se de argumentos da função `errbar()` (@fig-errorbar). Veja maiores detalhes na ajuda do *R* (`?errbar`).

```{r #fig-errorbar, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico de barras de erro", fig.pos="H"}
...
Será usada a variável `altura`, incluída da arquivo `mater` (veja início da @sec-tabelas), para a construção do histograma, executando:.

```{r #fig-histb, out.width="80%", out.height="80%", fig.align='center', fig.cap="Histograma básico", fig.pos="H"}
hist(mater$altura)
```

No histograma da @fig-histb, observam-se alguns problemas que devem ser melhorados para tornar a sua aparência mais elegante.
...
Acrescentado argumentos, modifica-se o aspecto do histograma (@fig-histmod):

```{r #fig-histmod, out.width="80%", out.height="80%", fig.align='center', fig.cap="Histograma modificado", fig.pos="H"}
hist(mater$altura,
     breaks = 12,
     ylim = c (0, 450),
     xlim = c (1.4, 1.9),
...
Eventualmente, para melhor comparar a distribuição dos dados, é interessante incluir uma curva normal sobreposta que servirá de indicador (@fig-histnormal). A distribuição normal será discutida mais adiante (@sec-normal).
...
```{r #fig-histnormal, out.width="80%", out.height="80%", fig.align='center', fig.cap="Histograma com curva normal sobreposta", fig.pos="H"}
...
Uma das utilidades dos componentes, é construir um histograma com os valores correspondentes as barras sobrepostos ao gráfico (@fig-histfreq).

```{r #fig-histfreq, out.width="80%", out.height="80%", fig.align='center', fig.cap="Histograma com frequência sobreposta", fig.pos="H"}
hist(mater$altura,
     breaks = 8,
     ylim = c (0, 450),
     xlim = c (1.4, 1.9),
...
Note que as informações deste gráfico são as mesmas de uma tabela de frequência construída com os mesmos dados. Maneiras diferentes de informar uma distribuição de frequência (veja a @sec-freq).

### Boxplot {#sec-bxp}

O *boxplot* descreve a distribuição de uma variável contínua exibindo o resumo de cinco números: mínimo, 1º quartil (percentil 25), mediana (percentil 50), 3ª quartil (percentil 75) e máximo (@fig-boxplot).

```{r #fig-boxplot, echo = FALSE,out.height= '70%', out.width = '70%', fig.align = 'center', fig.cap="Boxplot", fig.pos="H"}
 knitr::include_graphics("https://i.imgur.com/Dh2UszX.png")
```

#### Boxplot a partir de um vetor

Para construir um boxplot, serão usados os mesmos dados dos recém-nascidos a termo, filtrados do conjunto de dados `dadosMater.xlsx`, como realizado na seção da construção de um gráfico de barra de erro (@sec-errorbar). Os dados obtidos, novamente serão atribuídos a um objeto de nome `rnt`. A variável usada para construir o boxplot será `rnt$pesoRN`.

O *R* possui uma função no pacote básico denominada `boxplot()` que foi usada para construir o gráfico da Figura @fig-bxps. A função solicita vários argumentos que podem alterar a sua aparência e devem ser utilizados de acordo com necessidade:
...
```{r #fig-bxps, out.width="80%", out.height="80%", fig.align='center', fig.cap="Boxplot simples", fig.pos="H"}
...
Esse boxplot pode ser modificado (@fig-bxpmod), alterando alguns argumentos como colocação de um título no gráfico, e rótulos nos eixos e mudança na cor. Os argumento `cex.lab`, `cex.axis` e `cex.names` estabelecem o tamanho fontes. Por exemplo, para aumentar em 20%, usamos 1.2.

```{r #fig-bxpmod, out.width="80%", out.height="80%", fig.align='center', fig.cap="Boxplot modificado", fig.pos="H"}
boxplot (rnt$pesoRN, 
         col = "lightblue2", 
         main = "RN a termo", 
         ylab = "Peso do Recém-nascido (g)",
...
[Interpretação do boxplot]{.underline}

O boxplot nos fornece uma análise visual da posição, dispersão, simetria, caudas e valores discrepantes (*outliers*) do conjunto de dados (@fig-boxplot).
...
Quando se observa um boxplot, verifica-se que os mesmos ocultam a distribuição subjacente dos dados. Para resolver este “problema”, pode-se adicionar pontos ao gráfico, usando a função `stripchart()`. Esta função permite criar um gráfico de dispersão unidimensional sobreposto ao boxplot (@fig-boxstrip). Os comandos para esta ação são:

```{r #fig-boxstrip, out.width="80%", out.height="80%", fig.align='center', fig.cap="Boxplot com pontos de dispersão", fig.pos="H"}
boxplot (rnt$pesoRN, 
          col = "lightblue2", 
          ylab = "Peso do Recém-nascido (g)", 
          border = "black",
...
É possível representar os intervalos de confiança de 95% para a mediana em um boxplot (@fig-boxnotch), definindo o argumento `notch` como `TRUE`.

```{r #fig-boxnotch, out.width="80%", out.height="80%", fig.align='center', fig.cap="Boxplot modificado", fig.pos="H"}
boxplot (rnt$pesoRN, 
          col = "lightblue2", 
          ylab = "Peso do Recém-nascido (g)", 
          border = "black",
...
[Interpretação das estatísticas]{.underline}
...
Os boxplots são muito usados na comparação de grupos. A necessidade mais comum é ordenar as categorias de acordo com o aumento da mediana, mas isto é opcional. Permite identificar rapidamente qual grupo tem o maior valor e como as categorias são classificadas (@fig-bxpm).\
...
```{r #fig-bxpm, out.width="80%", out.height="80%", fig.align='center', fig.cap="Múltiplos boxplots", fig.pos="H"}
...
Pode-se fazer um entalhe (*notch*) que podem ser interpretados como um intervalo de confiança em torno dos valores medianos @fig-bxpnotch). É calculado pela fórmula :$mediana \pm 1.57\times IIQ/\sqrt{n}$. No nosso exemplo, observe que o entalhe nos meninos está um pouco acima do das meninas..

```{r #fig-bxpnotch, out.width="80%", out.height="80%", fig.align='center', fig.cap="Boxplots com entalhes", fig.pos="H"}
boxplot (rnt$pesoRN ~ rnt$sexo, 
         col = c("lightblue2", "pink"), 
         ylab = "Peso do Recém-nascido (g)", 
         xlab = "Sexo",
...
Para criar um boxplot horizontal @fig-bxph), usamos o argumento `horizontal = TRUE` e invertemos os rotulos dos eixos *x* e *y*.

```{r #fig-bxph, out.width="80%", out.height="80%", fig.align='center', fig.cap="Boxplots horizontais", fig.pos="H"}
boxplot (rnt$pesoRN ~ rnt$sexo, 
...
Um gráfico de dispersão (*Scatterplot*) exibe a relação entre duas variáveis numéricas (@fig-scatter). Cada ponto representa uma observação. Suas posições nos eixos *x* (horizontal) e *y* (vertical) representam os valores das duas variáveis.
...
```{r #fig-scatter, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico de dispersão", fig.pos="H"}
plot (x = rnt$compRN,
      y = rnt$pesoRN,
...
Este mesmo gráfico pode ser obtido, usando uma fórmula `y~x` e acrescentando o argumento `bty = "L"` (@fig-scatter1). Este argumento permite personalizar a caixa ao redor do gráfico.
...
```{r #fig-scatter1, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico de dispersão", fig.pos="H"}
plot (pesoRN ~ compRN,
      data = rnt,
      ylab = "Peso de Recém-nascido (g)",
      xlab = "Comprimento do Recém-nascido (cm)",
...
Como em qualquer outro gráfico, este também pode ser melhorado em seu aspecto, tornando os pontos sólidos e coloridos. O argumento `pch` estabelece o tipo de pontos (@fig-pch).

```{r #fig-pch, echo = FALSE, out.width = '65%', fig.align = 'center', fig.cap="Símbolo dos formatos", fig.pos="H"}
 knitr::include_graphics("https://i.imgur.com/8VWZSGk.png")
```

Na @fig-scatter, como os pontos estão aglomerados, devido a quantidade, é possível tentar espalhá-los, usando a função `jitter()` na variável `compRN` (@fig-jitter). O argumento 10 é variável e significa o grau de espalhamento:

```{r #fig-jitter, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico de dispersão com *jitter*", fig.pos="H"}
...
Inicialmente, será criado um vetor para representar as cores, de acordo com o sexo (meninos = azul; meninas = vermelho). Usa-se a função `unclass()` para discriminar os sexos (@fig-map). Acrescenta-se uma legenda para ilustrar a separação.

```{r #fig-map, out.width="80%", out.height="80%", fig.align='center', fig.cap="Mapeamento dos pontos de acordo com uma variável categórica", fig.pos="H"}
...
Uma linha reta de ajuste dos dados (@fig-reta) pode ser acrescentada usando a função `abline ()`, associada a função `lm ()`. Um modelo típico lm (*linear model*) tem o formato *resposta (*y*) \~ preditor (*x*)*. Mais detalhes sobre o modelo de ajuste linear na @sec-rls (regressão linear).

```{r #fig-reta, out.width="80%", out.height="80%", fig.align='center', fig.cap="Gráfico de dispersão com reta de ajuste", fig.pos="H"}
...
Os três componentes principais de cada gráfico do `ggplot2`são: dados, estética e geometria.
...
3.  **geom** são as funções que criam camadas, pelo menos uma, que descrevem como renderizar cada observação.

### Principais gráficos usando `ggplot2`

#### Gráfico de dispersão

Serão usados os dados coletados em um ambulatório pediátrico relacionados à idade e ao comprimento de 40 crianças entre 18 e 36 meses (20 meninos e 20 meninas), incluídos no arquivo `dadosReg.xlsx`. Eles podem ser baixados [aqui](https://github.com/petronioliveira/Arquivos/blob/main/dadosReg.xlsx). Salve o arquivo no seu diretório de trabalho e carrege-o com a função `read_excel()` do pacote `readxl`:
...
```{r #fig-scatter2, eval=TRUE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Gráfico de dispersão", fig.pos="H"}
ggplot(data = dados, 
       mapping = aes(x = idade, y = comp)) +
  geom_point()
```

A estética, com a função `aes()`, foi adicionada na função `ggplot()` (@fig-scatter2) e isto significa que ela será usada em todas as outras camadas que forem acrescentadas. Pode-se colocar a a estética dentro da função do `geom` específico e ,dessa forma, funcionárá apenas para ele.

O mesmo resultado da @fig-scatter2, pode ser obtido da seguinte maneira:
...
```{r #fig-scatter5, eval=TRUE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Gráfico de dispersão  com separação dos pontos por uma variável categórica (sexo)", fig.pos="H"}
ggplot(dados, aes(x = idade, y = comp, color = sexo)) +
  geom_point()
```

Na @fig-scatter5, as cores, escolhidas pelo padrão do *R*, estão separando os sexos (masculino e feminino). A legenda, à direita identifica, as cores dos sexos.\
O que acontece se colocarmos a cor fora da função `aes()`?

```{r #fig-scatter6, eval=TRUE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Gráfico de dispersão com a cor sendo especificada fora da aes().", fig.pos="H"}
ggplot(dados, aes(x = idade, y = comp), color = sexo) +
  geom_point()
```

Não acontece nada! O *R* ignora o código e retorna um gráfico com as sua cor padrão, a preta (@fig-scatter6).

Para mudar a cor dos pontos de acordo com a nossa escolha (@fig-scatter7), é necessário informar a cor no `geom` (`color = "cor"`).

```{r #fig-scatter7, eval=TRUE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Gráfico de dispersão com a cor dos pontos por escolha pessoal", fig.pos="H"}
ggplot(dados, aes(x = idade, y = comp)) +
  geom_point(color = "tomato")
```

Para aumentar o tamanho dos pontos usa-se o argumento `size =`[^06-descrevendodados-7] no `geom`; o formato é modificado, da mesma maneira, com o argumento `shape =`[^06-descrevendodados-8]. O tamanho do ponto pode também ser modificado de acordo com uma variável, como feito com o argumento `color = sexo` (@fig-scatter6), colocando o argumento dentro da função `aes()`, no `ggplot()`.

[^06-descrevendodados-7]: padrão = 1.5

[^06-descrevendodados-8]: http://www.sthda.com/english/wiki/ggplot2-point-shapes

O código a seguir, modifica o tamanho do ponto e o seu formato de acordo com o sexo.

```{r  #fig-scatter8, eval=TRUE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Gráfico de dispersão com pontos aumentados e com formatos diferentes de acordo com o sexo.", fig.pos="H"}
ggplot(dados, 
       aes(x = idade, y = comp, shape = sexo)) + 
  geom_point(color = "tomato", size = 3)
```

A saída (@fig-scatter8) mostra um triangulo e um ponto, correspondendo, respectivamente, ao sexo masculino e feminino com tamanhos duas vezes maior do que o padrão.

Será criado outro exemplo (@fig-scatter9) com o argumento `fill = sexo` para permitir a visualização dos sexos com preenchimento em cores diferentes, conforme o padrão do `ggplot2`. Observe que foi utilizado o argumento dentro da estética do `ggplot()`. O argumento `color=“black”` será colocado no `geom_point()`, para colocar uma borda preta ao redor de todos os pontos. O argumento `shape = 21` corresponde a um formato de pontos vazios.

```{r  #fig-scatter9, eval=TRUE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Gráfico de dispersão semelhante ao da Figura 5, apenas com pontos maiores.", fig.pos="H"}
ggplot(dados, aes(x = idade, y = comp, fill = sexo)) +
  geom_point(color = "black", size = 5, shape = 21) 
```

[Facetamento]{.underline}

Outra técnica para exibir variáveis categóricas adicionais em um gráfico é o facetamento. O facetamento cria gráficos dividindo os dados em subconjuntos e exibindo o mesmo gráfico para cada subconjunto (@fig-scatter10) . Para facetar um gráfico, basta adicionar uma especificação de facetamento com a função `facet_wrap()`, que recebe o nome de uma variável precedido pelo sinal `~`.

```{r #fig-scatter10, eval=TRUE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Facetamento: gráfico de dispersão por categorias.", fig.pos="H"}
ggplot(dados, aes(x = idade, y = comp)) +
  geom_point(fill = "tomato", color = "black", size = 5, shape = 21) +
  facet_wrap(~sexo)
```

Para observar um padrão dominante, como feito acima (@fig-scatter4), pode-se adicionar ao gráfico a função `geom_smooth(method = "lm")` para ajustar um modelo linear, para comparação [^06-descrevendodados-9]:

[^06-descrevendodados-9]: Observe que não foi usado o argumento `se = FALSE`. Por isso, aparece o intervalo de confiança de 95%.

```{r #fig-scatter11, eval=TRUE, warning=FALSE, out.width="60%", out.height="60%", fig.align="center", fig.cap="Reta de regressão com intervalo de confiança por categorias", fig.pos="H"}
ggplot(dados, aes(x = idade, y = comp)) +
  geom_point(fill = "tomato", color = "black", size = 5, shape = 21) +
  facet_wrap(~sexo) +
...
Neste caso (@fig-scatter11), observa-se que as inclinações das retas são praticamente iguais, indicando que o `sexo` não modifica a correlação entre variáveis.

#### Boxplot {#sec-geom_boxplot}

Na @sec-bxp foi mostrado o objetivo de um boxplot, agora, será construído um boxplot, usando o `geom_boxplot()` para comparar o comprimento das crianças de acordo com o sexo (@fig-ggbxp0):

```{r  #fig-ggbxp0, echo = TRUE,out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="FIGURA 14: Boxplot padrão do ggplot2", fig.pos="H"}
ggplot(dados, aes(x = sexo, y =  comp)) +
  geom_boxplot()
```

Este é um boxplot simples, mas facilmente, pode-se modificá-lo, tornando-o mais atraente [^06-descrevendodados-10]. Por exemplo, controlando a cor das caixas (@fig-ggbxp1) com o argumento `color` (ou `colour`) para modificar as bordas e `fill` para modificar o preenchimento das caixas. A adição do argumento `alpha` modifica a transparência de um geom. Os valores de `alpha` variam de 0 a 1, com valores mais baixos correspondendo a cores mais transparentes:

[^06-descrevendodados-10]: É claro, que este aspecto tem uma conotação pessoal. Em publicações, consulte as normas do periódico

```{r #fig-ggbxp1, echo = TRUE,out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Boxplots com as cores das caixas alteradas.", fig.pos="H"}
ggplot(dados, aes(x = sexo, y =  comp)) +
...
A escolha da cor pode ser realizada entre as 657 opções que o *R* oferta, consultando [Dealing with colors in ggplot2](https://r-graph-gallery.com/ggplot2-color.html) ou [Colors in R](chrome-extension://efaidnbmnnnibpcajpcglclefindmkaj/http://www.stat.columbia.edu/~tzheng/files/Rcolor.pdf) [^06-descrevendodados-11].

[^06-descrevendodados-11]: Veja mais detalhes na @sec-cores

O boxplot oculta a distribuição subjacente dos dados. Para resolver isto, pode-se adicionar pontos ao gráfico (@fig-ggbxp2) usando o `geom_jitter()`. Para evitar que os pontos fiquem muito espalhados, dificultando a visualização das categorias, usa-se o argumento `width` [^06-descrevendodados-12]. Um espalhamento de 10% (`width = 0.10`)para poucos pontos, como no exemplo, parece bom, para observar a distribuição dos mesmos.

[^06-descrevendodados-12]: O padrão é `width = 0.40`. Usando `width = 0`, teremos uma distribuição dos pontos em uma mesma posição, como o gráfico inicial do Boxplot

```{r #fig-ggbxp2, echo = TRUE,out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Boxplot com jitter.", fig.pos="H"}
ggplot(dados, aes(x = sexo, y =  comp)) +
  geom_boxplot(colour = "skyblue4", 
               fill = "skyblue", 
               alpha = 0.3) +
...
Continuando a alteração da aparência do boxplot, é posível modificar o seu formato para o formato clássico com “bigodes” terminando em “T” (Figura @fig-ggbxp3) e não um traço simples. Para isso, cria-se uma camada de barra de erro, usando a função `geom_errorbar()`, antes de `geom_boxblot()`. Assim, como o boxplot passa ser a camada mais superficial, ele impede que se visualize a barra de erro na caixa, desde que ele seja opaco (remover ou zerar o argumento `alpha`). A função `geom_errorbar()` normalmente é usada para barras de erro. No entanto, aqui ela está sendo utilizada com `stat = "boxplot"` [^06-descrevendodados-13], o que significa que os cálculos de estatística do boxplot serão aplicados à barra de erro. O argumento `width = 0.1` ajusta a largura das barras de erro, tornando-as mais estreitas.

[^06-descrevendodados-13]: O padrão é `stat = "identity"`, o que significa que os valores das barras de erro devem ser fornecidos diretamente no conjunto de dados, sem cálculos adicionais

```{r #fig-ggbxp3, echo = TRUE,out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Boxplot no ggplot2 com bigodes finalizando em T.", fig.pos="H"}
ggplot(dados, aes(x = sexo, y =  comp)) +
...
Para construir o gráfico de violino, serão usados os argumentos `trim = FALSE`, para não aparar caudas, e `draw_quantiles = c(0.25, 0.5, 0.75)`, para traçar os quartis (@fig-violino). No final, a função `theme()` será colocada para evitar que a legenda das categorias apareça, uma vez que ela é explicita no gráfico.

```{r #fig-violino, echo = TRUE,out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de violino com os quartis.", fig.pos="H"}
...
Uma alteração interessante que pode ser feita no gráfico de violino é colocar um boxplot (@fig-violino1), dentro do mesmo, faz o efeito do argumento `draw_quantiles`, usado na @fig-violino. Facilita a interpretação e, na opinião do autor, é mais elegante. O argumento `width = 0.5`, na função `geom_boxplot()`, estabelece a largura do boxplot.

```{r #fig-violino1, echo = TRUE,out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de violino com boxplot.", fig.pos="H"}
...
Para obter uma versão horizontal da @fig-violino1, chama-se a função `coord_flip()` [^06-descrevendodados-14] que permite inverter os eixos *X* e *Y* e, assim, tornar a interpretação mais intuitiva, mais amigável (@fig-violino2).

[^06-descrevendodados-14]: Esta função pode ser utilizada para outros gráficos, consulte a ajuda

```{r #fig-violino2, echo = TRUE,out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de violino horizontal.", fig.pos="H"}
ggplot(dados_fumo, aes(x=quantFumo, y=pesoRN, fill=quantFumo)) + 
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.5) +
  coord_flip() +
...
Como visto na @sec-histograma, o *histograma* é uma ferramenta gráfica que fornece informações sobre o formato da distribuição e da dispersão dos dados, permitindo verificar se existe ou não simetria. É usado para dados contínuos.
...
Para construir um histograma, será usada a variável `pesoRN` do banco de dados usado nos gráficos de violino (`dadosFumo.xlsx`). O histograma (@fig-hist) pode ser obtido por qualquer um dos códigos abaixo:
...
```{r #fig-hist, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Histograma padrão.", fig.pos="H"}
...
Junto com a alteração dos intervalos, vamos modificar a cor de preenchimento (`fill`) e bordas (`colour`) das barras (@fig-hist2).

```{r #fig-hist2, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Histograma modificado com o argumento binwidth = 100 e com preenchimento e bordas das barras customizadas.", fig.pos="H"}
ggplot(dados_fumo, aes(x = pesoRN)) + 
...
```{r #fig-hist3, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Histograma com curva normal sobreposta.", fig.pos="H"}
ggplot(dados_fumo) + 
  geom_histogram(aes(x = pesoRN, 
...
O gráfico da @fig-hist3, mostra que os pesos dos recém-nascidos se ajustam bem à curva normal.
...
A função `geom_bar()` permite delinear um gráfico de barras (@fig-bar). O exemplo será construído com a variável `quantFumo` do arquivo `dadosFumo.xlsx`.

```{r #fig-bar, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barras padrão do ggplot2.", fig.pos="H"}
ggplot(data = dados_fumo) +
  geom_bar(aes(x = quantFumo, 
               y = after_stat(count/sum(count))))
```

As cores de preenchimento das barras podem ser alteradas, de acordo com a variável categórica (@fig-bar1). As cores serão estabelecidas de acordo com o padrão do `ggplot2`:

```{r #fig-bar1, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barras com as cores das barras estabelecidas pelo ggplot2.", fig.pos="H"}
ggplot(data = dados_fumo) +
  geom_bar(aes(x = quantFumo, 
               y = after_stat(count/sum(count)),
               fill = quantFumo))
...
O gráfico retorna uma legenda, mostrando o que representa cada cor. Ela é desnecessária porque já está explicito, no eixo *X*, o que cada barra representa. Portanto, vamos remover a legenda (@fig-bar2)com a função `theme(legend.position = "none")`:

```{r #fig-bar2, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barras anterior sem legenda.", fig.pos="H"}
ggplot(data = dados_fumo) +
  geom_bar(aes(x = quantFumo, 
               y = after_stat(count/sum(count)),
               fill = quantFumo)) +
...
Essas cores do padrão do `ggplot2` podem ser modificadas com a função `scale_fill_manual()`que permite a customização das cores (@fig-bar3):

```{r #fig-bar3, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barras com cores customizadas.", fig.pos="H"}
ggplot(data = dados_fumo) +
  geom_bar(aes(x = quantFumo, 
               y = after_stat(count/sum(count)),
               fill = quantFumo)) + 
...
Observe que no gráfico da @fig-bar3, o eixo *y* contém um rótulo que é a fórmula para o cálculo da proporção e o eixo *x*, o nome da variável `quantFumo` que também é pouco esclarecedor. Os rótulos dos eixos são muito importantes em um gráfico, pois a sua leitura deve explicar o que o gráfico está apresentando. Nessa figura, o gráfico está mostrando a proporção da intensidade de tabagismo em um grupo de gestantes. Isto deve estar claro no gráfico. Para modificar o rótulo dos eixos, utiliza-se, com frequência, as funções `xlab()` e `ylab()` (@fig-bar4).

```{r #fig-bar4, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barras com os rótulos dos eixos alterados.", fig.pos="H"}
ggplot(data = dados_fumo) +
  geom_bar(aes(x = quantFumo, 
               y = after_stat(count/sum(count)),
               fill = quantFumo)) + 
...
O mesmo trabalho de alteração dos rótulos pode ser feito com a função `labs()` que faz , retornando um gráfico igual ao da @fig-bar4:

```{r #fig-bar5, eval=FALSE}
ggplot(data = dados_fumo) +
  geom_bar(aes(x = quantFumo, 
               y = after_stat(count/sum(count)),
               fill = quantFumo)) + 
...
A @fig-bar4 ficou melhor, mas ainda tem problemas. O rótulo de cada barra está como cada nível está escrito no banco de dados, por exemplo, a palavra `não` está sem acentuação. Isto pode ser corrigido, usando a função `scale_x_discrete()` com os argumentos `limits =` (coloca os níveis na ordem desejada) e `labels =` (coloca os novos nomes[^06-descrevendodados-15] na ordem estabelecida pelo argumento `limits =`):

[^06-descrevendodados-15]: Podemos aproveitar aqui para trocar os nomes ou , simplesmente, corrigir acentuação que, às vezes, não foi colocada nos níveis

```{r #fig-bar6, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barras com nome e ordem dos rótulos alterados.", fig.pos="H"}
ggplot(data = dados_fumo) +
  geom_bar(aes(x = quantFumo, 
               y = after_stat(count/sum(count)),
               fill = quantFumo)) + 
...
Observe que a barra azul das não fumantes foi trocada de posição, passando a ser a primeira, para colocar a intensidade do tabagismo em ordem crescente (@fig-bar6).
...
Agora, será desenhado um gráfico com boxplots que ilustrem a influência do tabagismo materno sobre o peso do recém-nascido. Será usado o mesmo banco de dados `dadosFumo.xlsx`. Além de usar todos os argumento da função `labs()`, se repetirá o que foi feito na construção do gráfico da @fig-bar6, alterando os nomes e a posição das não fumantes. O gráfico (@fig-ggbxp4) vai ser atribuído a um objeto denominado `bxp`:

```{r #fig-ggbxp4, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Boxplots mostrando o impacto do tabagismo materno no peso do recém-nascido", fig.pos="H"}
...
O gráfico da @fig-ggbxp4 está com bom aspecto, mas alguém poderia dizer que gostaria de alterar o tamanho da fonte do título e do subtítulo, mudando inclusive a cor. É exagero? Poder ser, mas é possível! Para isso, existe, no `ggplot2`, uma função denominada `theme()` com múltiplos argumentos que exercem uma grande quantidade ações. Sugere-se consultar a ajuda (`?theme()`)para maiores informações. Para realizar essas alterações, vamos começar com o objeto `bxp` [^06-descrevendodados-16] e adicionar os códigos com a função `theme()` para alterar a cor do título e subtítulo (@fig-ggbxp5):

[^06-descrevendodados-16]: Este procedimento de atribuir a um objeto os comandos que constroem um gráfico, facilita a digitação, pois não é necessário repetir os códigos da Figura \@ref(fig:ggbxp4)

```{r #fig-ggbxp5, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico anterior com pequenas alterações na fonte e cores do título e subtítulo", fig.pos="H"}
bxp +
...
No gráfico da @fig-ggbxp5, os pesos dos RNs estão dispostos em uma escala que varia a cada 1000 g. Para modificar esses limites, pode-se usar a função `scale_y_continuous()` para ter intervalos de 500 g:

```{r #fig-ggbxp6, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico anterior com modificação da escala do eixo Y", fig.pos="H"}
bxp +
  theme(plot.title = element_text(size = 14,
                                  face = "bold"),
        plot.subtitle = element_text(size = 12,
...
Agora, na @fig-ggbxp6, o eixo *Y* contém intervalos de 500 g. Este mesmo grafico já é um exemplo de modificação do eixo *X* com a função `scale_x_discrete()` para mudar a ordem dos níveis originais, realizado na construção do gráfico da @fig-ggbxp4.

#### Modificação da expansão

Voltando ao gráfico da @fig-bar6, observe que abaixo do valor 0 (zero) existe uma expansão. Isto, visualmente, é desagradável (pelo menos para o autor). Para que as barras tenham início exatamente no 0 (zero), pode-se empregar a função `scale_y_continuous()` com o argumento `expand = expansion (add = c(0,50))`, significando que não se expande nada abaixo do 0 e se adiciona 50 unidades para cima, criando uma margem superior (@fig-bar7).

```{r #fig-bar7, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barra sem a expansão abaixo de 0 (zero)", fig.pos="H"}
ggplot(data = dados_fumo) +
  geom_bar(aes(x = quantFumo, 
               y = after_stat(count/sum(count)),
               fill = quantFumo)) + 
...
Na @fig-bar7, a unidade do eixo *Y* encontra-se como um proporção `y = after_stat(count/sum(count)`. É possível modificar para porcentagem (@fig-bar8), empregando a função `percent_format()` do pacote `scales`. O código é praticamente igual, apenas acrescentar o argumento `labels = percent_format(accuracy = 0.1, decimal.mark = “,”)` dentro da função `scale_y_continuous()`.

```{r #fig-bar8, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barra com eixo Y em porcentagem", fig.pos="H"}
library(scales)
...
Na @sec-geom_boxplot, foi introduzido o uso de cores no R. Agora, apesar deste tema praticamente não ter limites, serão mostrados alguns princípios do manuseio das cores no `ggplot2`.

No gráfico de barras da @fig-bar8, as cores foram selecionadas com a função `scale_fill_manual()`. A escolha das cores pode ser feita especificando o seu nome em inglês. Essa escolha é pessoal. O *R* possui 657 cores integradas que permitem uma gama ampla de opções. Uma outra maneira de especificar as cores, é usar o sistema RGB ou hexadecimal. O código hexadecimal da cor branca é #FFFFFFF, da “gray58” é #949494, da “yellow4” é #999900, etc. Opcionalmente, a cor pode ser transparente, usando o formato “#RRGGBBAA”. `Alpha` refere-se à transparência de um `geom`. Os valores de `alpha` variam de 0 a 1, com valores mais baixos correspondendo a cores mais transparentes. `Alpha` também pode ser modificada por meio da estética de `colour` ou `fill` se qualquer uma das estéticas fornecer valores de cor usando uma especificação RGB.

Usando este mesmo gráfico, iremos mudar a cor, usando a cor cinza-claro para o preenchimento das barras (`fill = “gray70”`) e vermelho-escuro para bordas das barras (`color = “darkred”`), colocados fora da `aes()`, pois se forem colocadas dentro, as cores ficarão diferentes dentro do padrão do `ggplot2` (@fig-bar9).

```{r #fig-bar9, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barras com cores modificadas fora da estética do geom.", fig.pos="H"}
library(scales)
...
Como base, será usado o código que gerou o gráfico da @fig-ggbxp6 com alterações, usando a paleta do periódico *Lancet* (@fig-ggbxp7).

```{r #fig-ggbxp7, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico com paleta de cores do Lancet.", fig.pos="H"}
library(ggsci)
...
Será repetido o boxplot da @fig-ggbxp7, adicionando o `theme_bw()` (@fig-ggbxp8) e, após, o `theme_classic()`(@fig-ggbxp9).

```{r #fig-ggbxp8, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico da Figura 36 com o theme_bw().", fig.pos="H"}
bxp1 +
  theme_bw() +
  theme(legend.position = "none")
...
```{r #fig-ggbxp9, message=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico da Figura 36 com o theme_classic().", fig.pos="H"}
bxp1 +
  theme_classic() +
  theme(legend.position = "none")
```
...
Um gráfico de barra de erro (@fig-bar10) é uma ferramenta visual que mostra a variabilidade de dados em um ponto específico (veja também @sec-errorbar). Ele consiste em pontos ou barras que representam as médias (ou outras estatísticas) de um conjunto de dados, com linhas verticais (ou horizontais) que indicam o intervalo de confiança, o desvio padrão ou o erro padrão da média. Essas linhas verticais são conhecidas como "barras de erro". Usado para comparar as médias de diferentes grupos, mostrando a variabilidade dentro de cada grupo. É visto com frequência em pesquisas científicas e publicações para apresentar os resultados experimentais com suas respectivas variabilidades.
...
[Exploração e transformação dos dados]{.underline}
...
```{r #fig-bar10, message=FALSE, warning=FALSE, echo = TRUE, out.height= '60%', out.width = '60%', fig.align = 'center', fig.cap="Gráfico de barra de erro.", fig.pos="H"}
ggplot(resumo, 
