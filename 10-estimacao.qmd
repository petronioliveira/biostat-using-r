...
Os dados deste capítulo são os mesmo usados no @sec-cap9, incluídos no arquivo `dadosMater.xlsx`, considerando apenas os recém-nascidos a termo e a variável `altura` e `pesoRN`:
...
A seguir, será extraída, dessa população, uma amostra de *n* = 30 [^10-estimacao-1] e calculado os mesmas medidas resumidoras, que se constituirão nas estimativas da `amostra`:

[^10-estimacao-1]: Repetindo, é importante lembrar que toda vez que for extraída uma nova amostra, o resultado será um conjunto de números diferentes e, em consequência, a média será diferente. Por isso, se for importante repetir o mesmo resultado, deve-se usar a função `set.seed()`. Consulte a @sec-rnorm.
...
Onde *Z* é o valor crítico para o nível de confiança escolhido, obtido da tabela de distribuição normal padrão, e *me* é a margem de erro ($z_{0,975} \times erro \quad padrao$). Um intervalo de confiança de 95% significa que a área total sob a curva normal entre dois pontos em torno da média populacional, $\mu$, é igual a 95%, ou 0,95. A área das caudas é $\alpha$, ou seja, cada cauda á igual a $\frac{\alpha}{2}$ (@fig-twotailed)).

```{r #fig-twotailed, echo = FALSE, out.width = '70%', fig.align = 'center', fig.cap="Intervalo de Confiança de 95%.", fig.pos="H"}
...
Essa função pode ser salva no seu diretório e, quando necessária, pode ser ativada com a função `source()`, como visto na @sec-funcpropria. Com essa função fica fácil alterar o nível de confiança, por exemplo, para 99%. Isso mudará o *Z* crítico para:
...
Observando o IC95% e o IC99%, verifica-se que a amplitude do intervalo aumentou com o crescimento da confiança de 95% para 99%, porque houve um aumento na margem de erro (@fig-IC9599).

```{r #fig-IC9599, echo = FALSE, out.width = '70%', fig.align = 'center', fig.cap="Comparação do IC95% e IC99%.", fig.pos="H"}
...
O IC95% informa sobre a *precisão* com que a média amostral estima a média populacional desconhecida [^10-estimacao-2].

[^10-estimacao-2]: Anteriormente, mostrou-se a media populacional por uma questão didática. A regra é não se conhecer a média populacional, razão da importância do intervalo de confiança

Na @fig-vinte, são mostradas 20 amostras diferentes de tamanho *n* = 30, dessa população. Junto aparecem os intervalos de confiança de 95% construídos em torno dessas amostras. Observa-se que apenas uma amostra (em vermelho) não inclui a média populacional (linha tracejada vertical em azul). Pode-se afirmar com 95% de confiança que se forem extraídas muitas amostras do mesmo tamanho de uma população e construído intervalos de confiança de 95% em torno das médias dessas amostras, 95% desses intervalos de confiança incluirão a média populacional.
...
```{r #fig-vinte, echo=FALSE, fig.out="75%", fig.cap="Intervalos de confiança de 95% que mostra 20 replicações simuladas de amostras de n = 30 do peso do recém-nascido. Apenas um intervalo (em vermelho) não inclui a média populacional (linha vertical azul)."}
ggplot2::ggplot(dfx, 
                aes(x = sample.means, 
                    y = trial, 
                    xmin = lcl, 
...
```{r #fig-familia, echo=FALSE, warning=FALSE, out.width="80%", out.height="80%", fig.align="center", fig.cap="Curvas de distribuição t conforme o grau de liberdade comparadas à distribuição normal.", fig.pos="H"}
...
```{r #fig-bilateral, echo=FALSE, warning=FALSE, out.width="70%", out.height="70%", fig.align="center", fig.cap="Distribuição t com gl = 10, bilateral.", fig.pos="H"}
# 1º Gráfico
...
```{r #fig-unilateral, echo=FALSE, warning=FALSE, out.width="70%", out.height="70%", fig.align="center", fig.cap="Distribuição t com gl = 10, unilateral.", fig.pos="H"}
x <- seq (-4, 4, length = 1000)
y <- dnorm(x)
plot(x, y, axes = FALSE, type = 'l', xlab = '',
     ylab = '',main = "")
...
Serão utilizados nesta seção, os dados da altura de mulheres, obtidos na @sec-dadoscap10. Suponha-se que os parâmetros sejam desconhecidos. Para estimar esses parâmetros, selecionou-se uma amostra de n = 30 desse conjunto dados. Tomando essa amostra, calcula-se a sua média e o seu desvio padrão:
...
```{r #tbl-tabtz, echo=FALSE, message=FALSE, warning=FALSE}
...
*3ª Etapa*: Cálculo do erro padrão da proporção ($\sqrt \frac {\hat p \times \hat q}{n}$) e da margem de erro (veja também a @sec-popamostra):
