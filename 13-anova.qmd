...
[^1]: Pode ser usada também para comparar a média de duas populações e o resultado será o mesmo de um teste *t* para amostras independentes.
...
[^2]: No texto, *df1* e *df2* (em inglês, *df* = *degree of freedom*) foram traduzidos para o português como *gl1* e *gl2* (*gl* = graus de liberdade).
...
Os boxplots (@fig-bxpfumo) são uma maneira interessante de visualizar os dados, principalmente com o pacote `ggplot2`[^3] :

[^3]: Volte à @sec-ggplot2 para mais informações sobre o como fazer gráficos no `ggplot2.`
...
Este gráfico (@fig-normresiduos) pode ser contruído com a função `ggqqplot()` do pacote `ggpubr`.  
...
(2)	Os dados são apresentados como média e desvio padrão, na  @tbl-result).^[lembrar que um litro de cerveja (4,5%) = 5 unidades de alcool] 
