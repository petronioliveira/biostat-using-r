...
```{r #fig-estriol, echo = FALSE, out.width = '50%', fig.align = 'center', fig.cap="Correlação da excreção de estriol urinário e peso fetal", fig.pos="ht"}
...
```{r #fig-pesorenda, echo = FALSE, out.width = '50%', fig.align = 'center', fig.cap="Gráfico de dispersão sugerindo ausência de correlação.", fig.pos="ht"}
...
```{r #fig-coefcor, echo = FALSE, out.width = '70%', fig.align = 'center', fig.cap="Coeficiente de Correlação.", fig.pos="ht"}
...
Aqui, será usado o gráfico de dispersão (@fig-idadealtura), usando a função `geom_point()` do pacote `ggplot2`:

```{r #fig-idadealtura, message = FALSE, warning=FALSE, fig.align='center', out.height="70%", out.width="70%", fig.cap="Correlação entre a idade e a altura de uma criança."}
...
-   *Análise gráfica da homoscedasticidade*: Pode ser feita através dos gráficos diagnósticos para a regressão linear, utilizados para verificar se o modelo funciona bem para representar os dados. Uma forma de avaliar é verificar como as variâncias se comportam. Os gráficos diagnósticos são apresentados de quatro maneiras diferentes.[^1] Neste estágio, serão avaliados o primeiro e o terceiro tipo (@fig-homo1).

[^1]: Maiores detalhes sobre os testes diagnósticos podem ser encontrados em: https://data.library.virginia.edu/diagnostic-plots/

```{r #fig-homo1, message = FALSE, warning=FALSE, fig.align='center', out.height="70%", out.width="70%", fig.cap="Gráficos diagnósticos 1 e 3"}
...
```{r #tbl-coefr, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r #fig-retareg, echo = FALSE, out.width = '60%', fig.align = 'center', fig.cap="Reta de regressão, coeficiente angular e coeficiente linear.", fig.pos="ht"}
...
No exemplo usado no início (@sec-execor) , verificou-se que existe uma correlação linear entre o a idade e o comprimento de crianças, usando uma amostra de 40 crianças entre 18 e 36 meses. A correlação de Pearson foi muito forte (*r* = 0,96, *P* \< 0,00001). Esta relação linear pode ser descrita pela reta, mostrada na @fig-retareg1.

```{r #fig-retareg1, message = FALSE, warning=FALSE, fig.align='center', out.height="70%", out.width="70%", fig.cap="Reta de regressão"}
ggplot2::ggplot(dados, 
                aes(x = idade, 
                    y = comp,
                    color = "tomato")) +
...
Para se obter os resíduos, graficamente, pode ser usar os seguintes comandos que resultam na @fig-residuos.
...
```{r #fig-residuos, message = FALSE, warning=FALSE, fig.align='center', out.height="70%", out.width="70%", fig.cap="Resíduos"}
...
```{r #fig-diagreg, message = FALSE, warning=FALSE, fig.align='center', out.height="90%", out.width="70%", fig.cap="Gráficos diagnósticos"}
par(mfrow=c(2,2))
plot (mod_reg)
par(mfrow=c(1,1))
```
...
A saída retorna a estatística do teste de Shapiro-Wilk com um valor P = 0,655, mostrando que os dados se ajustam à distribuição normal. Pode-se também construir um gráfico QQ (@fig-diagreg1), usando a função `ggqqplot()` do pacote `ggpubr` que exibe o mesmo resultado.

```{r #fig-diagreg1, message = FALSE, warning=FALSE, fig.align='center', out.height="70%", out.width="70%", fig.cap="Gráfico QQ mostrando a normalidade dos resíduos do modelo de regressão linear"}
...
Na @sec-corpre, foi analisada a homocedasticidade , onde se viu que o teste de Breusch-Pagan, retornou um resultado de *P* = 0,8231, indicando que a variância permanece praticamente constante, havendo homocedasticidade nos resíduos.
...
```{r #tbl-samplesize, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r #fig-resreg, message = FALSE, warning=FALSE, fig.align='center', out.height="70%", out.width="70%", fig.cap="Resultado da regressão linear"}
